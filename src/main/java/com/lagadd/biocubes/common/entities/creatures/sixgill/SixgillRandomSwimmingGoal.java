...
public class SixgillRandomSwimmingGoal extends RandomSwimmingGoal {
    private final Sixgill sixgill;


    public SixgillRandomSwimmingGoal(Sixgill sixgill, double speed, int chance) {
        super(sixgill, speed, chance);
        this.sixgill = sixgill;
    }

    public boolean canUse() {
        if (!this.forceTrigger) {
            if (this.sixgill.getNoActionTime() >= 100) {
                return false;
            }

            if (this.sixgill.getRandom().nextInt(this.interval) != 0) {
                return false;
            }
        }
    ...
    protected Vec3 getPosition() {
        ...
        Vec3 vec3d = this.findRandomTarget(this.mob, 15, 8, !this.sixgill.getPassengers().isEmpty());

        for (int i = 0; vec3d != null && !this.mob.level.getBlockState(new BlockPos(vec3d)).isPathfindable(this.mob.level, new BlockPos(vec3d), PathComputationType.WATER) && i++ < 10; vec3d = this.findRandomTarget(this.mob, 10, 8, !this.sixgill.getPassengers().isEmpty())) {
        }
